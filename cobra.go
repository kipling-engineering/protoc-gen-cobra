...
func _{{.Parent.GoName}}{{.GoName}}Command(cfg *client.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
        ...
        {{range .Input.Fields}} {{/* Iterate over fields of the Input Request message (e.g., UpdateEntityRequest) */}}
            {{/* Check if this field is a Message and not the UpdateMask itself */}}
            {{if and (eq .Desc.Kind.String "MessageKind") (ne .GoName "UpdateMask") }}
                {{$messageContainerField := .}} {{/* This is the field like 'Entity entity' in 'UpdateEntityRequest' */}}
                {{range .Message.Fields}} {{/* Iterate over fields of the message type (e.g., fields of 'Entity') */}}
                    {{$actualField := .}} {{/* This is an actual data field like 'DisplayName' or 'Count' */}}
                    {{/* Construct the flag name as generated by walkField: cfg.FlagNamer("MessageContainerFieldGoName ActualFieldGoName") */}}
                    {{/* e.g., cfg.FlagNamer("Entity DisplayName") */}}
                    {{/* Note: This assumes TitleCase/PascalCase from GoNames. Adjust if FlagNamer is different. */}}
                    if cmd.Flags().Changed(cfg.FlagNamer(printf "%s %s" $messageContainerField.GoName .GoName)) {
                        {{/* Construct the path for FieldMask: "message_container_field_name.actual_field_name" */}}
                        {{/* e.g., "entity.display_name" */}}
                        updateMaskPaths = append(updateMaskPaths, "{{$messageContainerField.Desc.Name}}.{{.Desc.Name}}")
                    }
                {{end}}
            {{end}}
        {{end}}
...
var (
	...
	knownTypes   = map[protogen.GoIdent]struct{ Type, Parse, Value, Slice string }{
		...
		fieldMaskPkg.Ident("FieldMask"):  {"fieldMask", "ParseFieldMaskE", "FieldMaskVar", "FieldMaskSliceVar"},
...
func walkField(g *protogen.GeneratedFile, fld *protogen.Field, path, postSetCode []string, deprecated bool, visited map[protogen.GoIdent]bool) []string {
	// If the field is google.protobuf.FieldMask and named "UpdateMask", skip generating flags for it.
	// It will be populated automatically.
	if fld.Message != nil && string(fld.Message.Desc.FullName()) == "google.protobuf.FieldMask" && fld.GoName == "UpdateMask" {
		return nil
	}
	target := "req"
	if len(path) > 0 {
