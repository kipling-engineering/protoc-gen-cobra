...
// FieldInfoForMask holds information needed to generate field mask update logic.
...
func _{{.Parent.GoName}}{{.GoName}}Command(cfg *client.Config) *cobra.Command {
	...
	cmd := &cobra.Command{
        ...
        {{range .AllFieldInfos}}
            if cmd.Flags().Changed(cfg.FlagNamer("{{.FlagCheckPath}}")) {
                updateMaskPaths = append(updateMaskPaths, "{{.FieldMaskPath}}")
            }
        {{end}}
...
func walkField(g *protogen.GeneratedFile, fld *protogen.Field, path, postSetCode []string, deprecated bool, visited map[protogen.GoIdent]bool) []string {
	...
	if f := flagFormat(g, fld); f != "" {
		...
		if fld.Desc.IsList() {
			...
			// limited map support
		} else if visited[fld.Message.GoIdent] { // Corrected cycle check
			// cycle detected for fld.Message.GoIdent
		} else {
			// No cycle for fld.Message.GoIdent at this point in the path
			if oneof {
			...
			// Prepare subVisited map for the recursive call
			subVisited := make(map[protogen.GoIdent]bool, len(visited))
			for k, v := range visited {
				subVisited[k] = v
			}
			subVisited[fld.Message.GoIdent] = true // Mark current field's message type as visited for the deeper path
